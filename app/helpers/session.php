...
function flash($name = '', $message = '', $class = 'alert alert-success text-center mt-5') : void {
...
function saveRoom(DBUtils $utils) : void {
	$_SESSION['booking'] = [
		'room_type' => preg_replace('/(\w+)([A-Z])/U', '\\1 \\2',$_GET['room_type']),
		'services' => isset($_GET['services' . $_GET['room_type']]) ? mapServicesToCosts($_GET['services' . $_GET['room_type']],$utils) : null,
